...
const VintageCard = ({ title, imageUrl, buttonText, fontFamily = "'Rubik Bubbles', cursive", hoverText, applyHover = false }) => {
  ...
  return (
    <a href="#" className={`group relative block ${applyHover ? 'hover:opacity-100' : ''}`}>
      ...
      <div className={`absolute inset-0 p-2 sm:p-4 md:p-6 lg:p-8 flex items-end justify-center text-center z-30 transition-all ${applyHover ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'} group-hover:translate-y-0 group-hover:opacity-100 xs:block-hidden`}>
        <p className="text-white text-sm sm:text-lg md:text-2xl xs:text-xl">{hoverText}</p>
      </div>
