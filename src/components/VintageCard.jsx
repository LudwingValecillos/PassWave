...
const VintageCard = ({ title, imageUrl, buttonText, backgroundColor, fontFamily }) => {
  return (
    ...
    >
      ...
      <motion.h2
        className="absolute bottom-12 left-0 w-full text-left text-xl font-serif text-white z-10 px-4" // Asegura que ocupe el ancho total
        initial={{ x: '-100%' }} // Comienza fuera de la pantalla a la izquierda
        animate={{ x: ['-100%', '100%'] }} // Se mueve de izquierda a derecha
        transition={{ duration: 15, ease: 'linear', repeat: Infinity }} // Movimiento continuo
        style={{ fontFamily }}
      >
        {title}
      </motion.h2>
      ...
      <motion.h2
        className="absolute bottom-4 right-0 w-full text-right text-xl font-serif text-white z-10 px-4" // Asegura que ocupe el ancho total
